...
class VoteCounter
  ...
  def initialize
    @votes = 0
    @votes_hash = {up: 0, down: 0, left: 0, right: 0, l: 0, r: 0, start: 0, select: 0, a: 0, b: 0, x: 0, y: 0, sleft: 0, sup: 0, sdown: 0, sright: 0}
  end
  def vote(v)
...
class OverSeer
 ...
 #{spacing} sleft: #{hash[:sleft]}, sright: #{hash[:sright]}, sup: #{hash[:sup]}, sdown: #{hash[:sdown]}"
